...
public class Steganography {
    ...
    */
    ...
    public static void hideText(Picture source, String s) {
        ...
        ArrayList<Integer> encryptedMessage = encodeString(s);
        ArrayList<Integer> encryptedBits = new ArrayList<Integer>();
        for(int i = 0; i < encryptedMessage.size(); i++) {
            int[] bits = getBitPairs(encryptedMessage.get(i));
            for(int j = 0; j < bits.length; j++) {
                encryptedBits.add(bits[j]);
            }
        }
    }
